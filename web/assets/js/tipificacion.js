...
let optRegistrosEfectivos = ['Selecciona una opción','Acepta renovación', 'Agenda Llamada', 'No acepta renovación'];
            ...
            selectSecondTip.addEventListener('change', function() {
                switch (this.value) {
                    case '1':
                        selectThirdTip.removeAttribute('disabled');
                        selectThirdTip.innerHTML = '';
                        optRenovacion.forEach(function(element, index) {
                            let renovacion = document.createRange().createContextualFragment(`<option value="${index}">${element}</option>`);
                            selectThirdTip.append(renovacion);
                        });
                        break;
                    case '2':
                        selectThirdTip.removeAttribute('disabled');
