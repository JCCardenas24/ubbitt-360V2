...
$(function () {
    $('#resumen-renovacion_side_menu').addClass('font-weight-bold');
...
$('#beyond-renovacion-carga-base-datos-tab').on(
    'shown.bs.tab',
    function (event) {}
);
...
function findSummaryGraphData(start, end) {
    $.ajax({
        url: '/ubbitt-beyond/find-renewal-summary-graph-data',
        type: 'POST',
        dataType: 'json',
        data: {
...
function findSummaryDetailData(start, end) {
    $.ajax({
        url: '/ubbitt-beyond/find-renewal-summary-detail-data',
        type: 'POST',
        dataType: 'json',
        data: {
            'SearchByDateForm[startDate]': start.format('YYYY-MM-DD'),
...
function updateConcentrateKpis(kpis, moneyFormatter) {
    ...
    $('#total-renewed-sale-amount').text(
        moneyFormatter.format(kpis.total_renewed_sale_amount).replace('.00', '')
    );
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            $.each(response.callsRecords, (index, callRecord) => {
                $('#ubbitt-beyond-renewal-calls-table tbody').append(
                    createCallRecordRow(callRecord)
                );
            });
            updatePaginator(
                '#ubbitt-beyond-renewal-calls-paginator',
                page,
...
function reportsListCallback(start, end, label, page = 1) {
    ...
    $.ajax({
        ...
        data: {
            ...
            'SearchByDateForm[submodule_origin]': 'renewal',
            'SearchByDateForm[type]': report_type,
        },
        success: (response) => {
            $('#beyond-renewal-reports-table tbody').html(null);
            $.each(response.reportsRecords, (index, reportRecord) => {
                $('#beyond-renewal-reports-table tbody').append(
                    createReportRecordRow(reportRecord)
                );
            });
            updatePaginator(
                '#beyond-renewal-reports-paginator',
                page,
                parseInt(response.totalPages),
                (page) => {
...
$('#beyond-renewal-reports-table tbody').on(
    'click',
    '.btn-delete-report',
    function (evt) {
        evt.preventDefault();
