...
function treeBoxes(urlService, jsonData) {
    ...
    function drawTree(jsonData) {
        ...
        let maxDepth = 0;
        let maxTreeWidth = breadthFirstTraversal(
            tree.nodes(root),
            function (currentLevel) {
                maxDepth++;
                currentLevel.forEach(function (node) {
                    if (node.type == 'type1') node.color = navy;
                    if (node.type == 'type2') node.color = salmon;
                    if (node.type == 'type3') node.color = yellow;
                    if (node.type == 'type4') node.color = red;
                    if (node.type == 'type5') node.color = aqua;
                    if (node.type == 'type6') node.color = orange;
                    if (node.type == 'type7') node.color = gray;
                    if (node.type == 'type8') node.color = black;
                });
            }
        );
    ...
    function update(source) {
        ...
        let nodesTooltip = nodeGroupTooltip
            .selectAll('g')
        ...
        let nodeEnterTooltip = nodesTooltip
            .enter()
            .append('g')
            .attr('transform', function (d) {
                return 'translate(' + source.y0 + ',' + source.x0 + ')';
        ...
        nodeEnter
            ...
            .html(function (d) {
                return (
                    ...
                    'px; height: ' +
                    (rectNode.height - rectNode.textMargin * 2) +
                    'px;" class="mini-tree_card">' +
                    '<span class="ttl_categoria">' +
                    d.ttl_categoria +
                    '</span>' +
                    '<br>' +
                    '<small>' +
        ...
        let linkTooltip = linkGroupToolTip
            .selectAll('g')
